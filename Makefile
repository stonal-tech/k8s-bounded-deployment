...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name k8s-bounded-deployment-builder
	$(CONTAINER_TOOL) buildx use k8s-bounded-deployment-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm k8s-bounded-deployment-builder
	rm Dockerfile.cross
...
.PHONY: template
template: manifests kustomize ## Render the Kubernetes manifests that would be deployed (kustomize build) to stdout.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
